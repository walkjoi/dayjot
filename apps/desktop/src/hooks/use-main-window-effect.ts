 ...
 * update checks, the OS deep-link intake. A secondary note window mounting a
 * second instance would double-run capture passes, git work, and index
 * writes, so new background controllers must come through here (or carry an
 * explicit `isMainWindow()` gate). The window-ownership rule itself is
 * documented on `lib/windows/window-role.ts`.
 *
