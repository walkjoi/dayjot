...
import { Bookmark, ChartLine, ListChecks, SquarePen } from 'lucide-react'
      ...
      <div className="flex flex-none flex-col">
        ...
        <nav aria-label="Primary" className="mt-6 space-y-1 px-4">
          ...
          <SidebarItem
            icon={
              <span className={lucideBox}>
                <Bookmark aria-hidden strokeWidth={1.75} className="size-4" />
              </span>
            }
            label="Keepsakes"
            binding={keybindingFor('nav.keepsakes') ?? undefined}
            active={route.kind === 'keepsakes'}
            onClick={() => void runCommand('nav.keepsakes', context)}
          />
          <SidebarItem
            icon={
              <span className={lucideBox}>
