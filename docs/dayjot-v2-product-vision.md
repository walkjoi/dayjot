# DayJot V2 Product Vision

This document describes the intended direction for DayJot V2: an open-source, local-first, markdown-native rewrite of DayJot that preserves the strongest ideas from V1 while making substantial architectural and product changes.
...
- [DayJot V2 Grounding Brief](./dayjot-v2-grounding-brief.md)
- [DayJot V2 Indexing Strategy](./dayjot-v2-indexing-strategy.md)
- [DayJot V2 Sync Strategy](./dayjot-v2-sync-strategy.md)

## Product Thesis

DayJot V2 should be a beautiful, local-first, AI-native markdown app for personal memory.

The app should preserve DayJot's core insight: people capture life chronologically, then understand it associatively. Daily notes remain the default capture surface, backlinks remain the organizing primitive, and search remains the recall layer. The major change is that V2 should be built on open, inspectable, portable markdown files instead of an encrypted cloud-first note database.

V2 should feel like DayJot, but it should be easier to trust, easier to extend, easier to back up, and easier for AI to work with.
...
DayJot V2 should continue to prioritize backlinks over folders as the primary organization model.
...
- Save the current browser page into DayJot without choosing a filing system first.
...
Keyboard operation is part of DayJot's identity. V2 should keep this.
...
### No DayJot-Hosted APIs

V2 should not depend on DayJot-hosted application APIs.
...
- Link enrichment should not use a DayJot-hosted replacement for V1's link description API.
...
This is a hard trust and architecture principle, not merely a launch simplification. Future business models should not assume DayJot operates proprietary note, AI, sync, capture, or link-description APIs for the local-first core product.
...
Obsidian is the strongest adjacent proof that users trust a local markdown vault. DayJot V2 should learn from Obsidian without becoming an Obsidian clone.

The lesson is not that DayJot should maximize plugins or configurability. The lesson is that open files create trust, and a coherent first-party intelligence layer can make those files feel alive.

### What To Borrow

V2 should borrow these Obsidian strengths:

- **Boring vault durability**: markdown files and attachments should remain inspectable, portable, and useful outside DayJot.
...
DayJot V2 should make AI-native behavior a coherent product layer:
...
DayJot V2 should avoid making users assemble an AI note-taking system from plugins. The core product should own context selection, retrieval, patch review, privacy boundaries, and provenance from the start.
...
- Users can back up the folder without DayJot.
- Users can inspect and edit files outside DayJot.
...
The local database should be treated as a cache/projection unless a specific field cannot safely live in markdown. Any non-rebuildable local state should be deliberately justified. See [DayJot V2 Indexing Strategy](./dayjot-v2-indexing-strategy.md) for the proposed projection and vector-index model.

SQLite and other generated local state should live under an ignored `.dayjot/` workspace directory. This keeps the workspace self-contained while preventing binary indexes, credentials, and transient state from being committed to GitHub or synced as durable note content.
...
The editor should be opinionated, calm, and fast. It should preserve DayJot's minimal feel while making markdown approachable.
...
The AI should have editing capabilities over notes, including multi-note patches. Edits should be represented as patchsets. Risky, destructive, or broad edits require review. Low-risk patchsets may be auto-applied only after DayJot has created a local checkpoint and only when locked notes are excluded.
...
But the product should not require DayJot-operated AI infrastructure to be useful. Any cloud AI call should go directly from the app to the chosen model provider, using user-owned credentials or explicit provider configuration.
...
V2 should include basic link capture. This is narrower than a full browser clipper, but it is still part of DayJot's daily-first capture spine.

The V1 implementation has a useful shape to preserve: browser capture creates a link record, a client-side operation turns that into a link note, and the daily note gets a `[[Links]]` entry pointing at the captured item. The V1 implementation also calls `link-description-api.vercel.app/describe` for an AI-generated summary. V2 should preserve the product behavior, but not the DayJot-hosted API shape.
...
The bridge should be an implementation adapter, not a product assumption. Native messaging is the preferred first spike for Chrome because it is an official extension-to-native-app channel and avoids exposing a local HTTP port. A loopback HTTP bridge can remain a fallback if screenshot payload size, streaming, or platform packaging makes native messaging awkward. A `dayjot://` deep link is acceptable only as a URL-only fallback because it is too limited for screenshots, structured metadata, retries, and reviewable errors. A DayJot-hosted cloud relay should not be used for the core capture path.
...
AI enrichment must obey the same privacy model as the rest of V2. If the target note or capture is marked `private: true`, DayJot must not send the URL contents, screenshot, selected text, or resulting note content to cloud AI. The product can still save the raw link locally without AI enrichment.
...
> TypeScript frontend in a Rust native shell, with the `dayjot` CLI bundled as a
...
The app should support a user-controlled backup path without requiring a DayJot account or DayJot-operated infrastructure.
...
GitHub setup should let the user choose a repository rather than assuming DayJot creates a managed private repo. DayJot should still configure safe defaults, ignores, and recovery behavior where possible.

### Adapter-Normalized Conflict Resolution

DayJot should normalize sync conflicts into a generic conflict model. Git conflicts, iCloud Drive duplicate files, local-folder conflicts, and future provider/protocol conflicts should all become the same kind of DayJot conflict before they reach the UI or AI.

Git becomes more plausible as a sync layer if DayJot owns the conflict-resolution experience. The AI can treat every conflict like a markdown merge task, then DayJot can translate the accepted resolution back into adapter-specific operations.
...
| CLI                | Read/discovery commands such as `dayjot search`, `dayjot show`, `dayjot today`, and path lookup. |
...
| DayJot-hosted sync      | Not part of the V2 baseline. The core product should not depend on DayJot-hosted APIs.                            |
...
| DayJot-hosted application APIs            | Drop. Provider calls should go directly from the app to the chosen provider.                 |
...
- **Workspace**: a local folder/vault with `daily/`, `notes/`, `assets/`, and ignored `.dayjot/`.
- **Source of truth**: markdown files.
- **Daily notes**: `daily/YYYY-MM-DD.md`.
- **Note identity**: readable filenames plus stable frontmatter IDs.
- **Projection layer**: SQLite under ignored `.dayjot/`, rebuilt from files wherever practical.
...
- **Network model**: no DayJot-hosted APIs for the core product; external calls go directly to user-approved providers such as LLM providers, GitHub, Git remotes, iCloud Drive, or cloud transcription providers.
...
- **Secrets**: per-device OS keychain or secure storage; never markdown, Git, or `.dayjot`.
...
- What command/skill subset should `~/.agents` discovery expose inside DayJot?
- What business model, if any, belongs in the long-term product?
- What is the eventual V1 migration path?

## Definition Of Success

DayJot V2 succeeds if a user can:
...
The product should feel like DayJot's memory model survived, but the substrate became open, local, markdown-native, and AI-ready.
