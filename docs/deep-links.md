...
The desktop app registers the `dayjot://` URL scheme, so other apps,
...
`dayjot://note/Project%20X` works too, via title resolution.

## Write links

```text
dayjot://append?text=call%20the%20bank    a bullet on today's daily note
dayjot://task?text=buy%20milk             an open task on today's daily note
```

A URL scheme is a world-invokable surface — any web page can attempt
`dayjot://` — so writes are deliberately narrow: **one line of plain text,
...
lands through the capture inbox (`.dayjot/inbox/`) — the same audited drain
...
- All flavors (DayJot, DayJot Beta, DayJot Dev) register `dayjot://`;
...
- A `dayjot://` link clicked *inside* the app (a note body, chat, a backlink
...
- iOS registers the scheme too (`CFBundleURLTypes` in `ios.project.yml`), but
  only for one **mobile-native verb**: `dayjot://record-audio`, the
...
resolves for scripts (`dayjot show`, `dayjot path`); the scheme navigates
and captures. `dayjot open <note>` bridges the two — it resolves like
`dayjot show` and shells out to the scheme URL, preferring the frontmatter
